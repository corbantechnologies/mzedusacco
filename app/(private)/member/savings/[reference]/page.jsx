...
function SavingsDetail() {
    ...
    if (!saving || !member) return <div className="p-8 text-center text-muted-foreground">Savings account not found.</div>;

    return (
        <div className="min-h-screen bg-gray-50/50">
            <div className="mx-auto p-4 sm:p-6 max-w-7xl space-y-6">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <div className="flex items-center gap-3 mb-1">
                            <h1 className="text-3xl font-bold text-gray-900">{saving.account_type}</h1>
                            <Badge variant={saving.is_active ? 'default' : 'secondary'}
                                className={saving.is_active ? 'bg-[#045e32]' : ''}>
                                {saving.is_active ? "Active" : "Inactive"}
                            </Badge>
                        </div>
                        <p className="text-muted-foreground font-mono">{saving.account_number}</p>
                    </div>
                            ...
                            <Card>
                                ...
                                <CardContent className="space-y-4">
                                    <div className="flex justify-between py-2 border-b">
                                        ...
                                        <span className="font-bold text-2xl text-[#045e32]">{formatCurrency(saving.balance)}</span>
                                    </div>
                                    <div className="flex justify-between py-2 border-b">
                                        <span className="text-muted-foreground">Interest Rate</span>
                                        <span className="font-semibold">{saving.account_type_details?.interest_rate}% p.a</span>
                                    </div>
                                    <div className="flex justify-between py-2 border-b">
                                        <span className="text-muted-foreground">Currency</span>
                                        <span className="font-semibold">{saving.deposits?.[0]?.currency || 'KES'}</span>
                                    </div>
                                </CardContent>
                            </Card>
                            ...
                            <Card>
                                ...
                                <CardContent className="space-y-4">
                                    <div className="flex justify-between py-2 border-b">
                                        ...
                                        <span className="font-semibold">{saving.account_type_details?.name}</span>
                                    </div>
                                    <div className="flex justify-between py-2 border-b">
                                        <span className="text-muted-foreground">Date Opened</span>
                                        <span className="font-semibold">{formatDate(saving.created_at)}</span>
                                    </div>
                                    <div className="flex justify-between py-2 border-b">
                                        <span className="text-muted-foreground">Last Updated</span>
                                        <span className="font-semibold">{formatDate(saving.updated_at)}</span>
                                    </div>
                                </CardContent>
                            </Card>
                        </div>
                                            ...
                                            <TableRow key={i}>
                                                ...
                                                <TableCell>
                                                    <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium 
                                                        ${t.type === 'Withdrawal' ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'}`}>
                                                        {t.type}
                                                    </span>
                                                </TableCell>
